...
class ActorCriticSG(nn.Module):
    ...
    def act(self, obs: TensorDict, **kwargs: dict[str, Any]) -> torch.Tensor:
        ...
        return self.clipping_layer(act)
    ...
    def get_actions_log_prob(self, actions: torch.Tensor) -> torch.Tensor:
        gaussian_actions = self.inverse_tanh(actions)
        return (self.distribution.log_prob(gaussian_actions) - torch.log(1 - (actions * actions) + 1e-6)).sum(dim=-1)
