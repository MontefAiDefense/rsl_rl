...
class PPO:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.optimizer = optim.AdamW(self.actor_critic.parameters(), lr=learning_rate, weight_decay=1e-5)
    ...
    def update(self):
        ...
        with torch.autograd.set_detect_anomaly(True, check_nan=True):
            for (
                ...
                if torch.isnan(actions_batch).any() or torch.isinf(actions_batch).any():
                    raise RuntimeError("actions_batch nan")
                ...
                # if torch.isnan(mu_batch).any():
                #     print("mu_batch nan")
                #     exit()
                sigma_batch = self.actor_critic.action_std
                # if torch.isnan(sigma_batch).any():
                #     print("sigma_batch nan")
                #     exit()
                entropy_batch = self.actor_critic.entropy
                # if torch.isnan(entropy_batch).any():
                #     print("entropy_batch nan")
                #     exit()
                ...
                if torch.isnan(actions_log_prob_batch).any():
                    raise RuntimeError("actions_log_prob_batch nan")
                delta = actions_log_prob_batch - old_actions_log_prob_batch.squeeze()
